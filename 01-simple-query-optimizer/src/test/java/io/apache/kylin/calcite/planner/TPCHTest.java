...
class TPCHTest {
    ...
    void runSQL(String SQL) throws Exception {
        Schema schema = new TpchSchema(0.01, 1, 1, false);
        Optimizer optimizer = Optimizer.create("tpch", schema);
        ...
        RelNode x = optimizer.beforeOptimize(relTree, relTree.getTraitSet().plus(EnumerableConvention.INSTANCE));
        print("AFTER sub-query", x);
        System.out.println(toSql(x));

        RelNode optimizerRelTree =
                optimizer.optimize(relTree, relTree.getTraitSet().plus(EnumerableConvention.INSTANCE), false);
    ...
    private void print(String header, RelNode relTree) {
        ...
        RelWriterImpl relWriter =
                new RelDotWriter(new PrintWriter(sw), SqlExplainLevel.DIGEST_ATTRIBUTES, true);
    ...
    private String toSql(RelNode root) {
        return toSql(root, SqlDialect.DatabaseProduct.MYSQL.getDialect());
    }
    ...
    Stream<DynamicTest> tpchTest() {
        return IntStream.range(0, 22).
                mapToObj(i -> dynamicTest("tpc_" + (i + 1), () -> runSQL(QUERIES.get(i))));
    }
}
